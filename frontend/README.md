...
회원가입/로그인/로그아웃과 퀴즈봇 화면. React + TypeScript(Vite)로 만들었고,
백엔드 없이 Firebase(Authentication, Firestore)만 쓴다. 안드로이드 앱과 같은
Firebase 프로젝트라 앱/웹이 같은 계정과 데이터를 공유한다.

스타일은 목업(`../src/main/resources/static/index.html`)에서 색·폰트·버튼·카드 규칙만
`src/styles/app.css`로 옮겨왔다. 팀원 웹과 합칠 때 갈아끼우면 된다.
...
`.env.local`이 비어 있으면 화면 위에 빨간 경고가 뜨고 로그인이 안 된다.
값은 Firebase 콘솔 → 프로젝트 설정 → 내 앱 → 웹 앱 → "SDK 설정 및 구성"에서 복사한다.

빌드는 `npm run build` (`dist/`), 결과 확인은 `npm run preview`.
...
- Vite라서 `import.meta.env.VITE_*`를 쓴다 (CRA의 `process.env.REACT_APP_*` 아님).
  env 파일은 `frontend/.env.local`.
- 비밀번호 확인 일치와 8자 이상은 보내기 전에 화면에서 검사한다. Firebase 자체 최소 길이는 6자.
- 가입하거나 구글 로그인하면 Firestore `users/{uid}`에 `{ name, email, createdAt }`를 merge로 쓴다.
  필드 이름이 안드로이드 앱의 `users`와 맞는지 확인 필요.
- 퀴즈봇은 로그인해야 들어간다. 안 되어 있으면 로그인 안내가 뜬다.
- 로그인 성공 후 어디로 갈지는 팀원 웹 담당이라, 지금은 "환영합니다"만 띄운다.

## Firebase 콘솔에서 해야 하는 것

1. 웹 앱 등록(`</>`) → config를 `.env.local`에 넣기
2. Authentication → 로그인 방법 → 이메일/비밀번호, Google 둘 다 켜기
   (Google은 지원 이메일 지정, 승인된 도메인에 `localhost` 있는지 확인)
3. Firestore 만들고 규칙 작성 — `users`, `quizzes`는 본인(uid) 문서만 읽기/쓰기
4. 팀원을 프로젝트에 편집자로 초대
5. 안드로이드에서 만든 계정으로 웹 로그인 되는지 확인
